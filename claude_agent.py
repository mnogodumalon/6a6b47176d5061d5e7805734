...
applookup target app_ids. Those are verified facts and quoting them is the point of that file.
   - NOTHING ELSE. The subagent's own prompt makes it Read `.claude/skills/intent-ui/SKILL.md` before it \
...
not read in .entity_summary: a brief that told the subagent to import `EnrichedKunden` named a type \
that does not exist.
...
or say explicitly that all records qualify and why.
      - COPY VERBATIM from .entity_summary: module paths, service method names, the `!` required markers, \
applookup target app_ids. Those are verified facts and quoting them is the point of that file.
      - NOTHING ELSE. The subagent's own prompt makes it Read `.claude/skills/intent-ui/SKILL.md` before \
