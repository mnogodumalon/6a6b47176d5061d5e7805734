...
auto-set derivable fields (running numbers, start status, today-dates) instead of asking; show + warn \
against every numeric guardrail the brief names; every selection list carries at least one derived stat.
...
async def _build_one_intent_page(flow: dict, index: int) -> dict:
    ...
    options = _agent_options(
        ...
        # The intent-ui skill lives in .claude/skills — without the project
        # source the session cannot find it and starts guessing contracts.
...
@tool(
    ...
    "There is no other way to create intent pages.",
    {
        ...
        "properties": {
            "flows": {
                # Live-proven repeatedly (this branch's 408s Handwerk-Pro run,
                # and twice on actions-on-build): the orchestrator stringifies
...
async def _build_intent_pages(args: dict) -> dict:
    ...
    print(f"[KLAR] Intent fan-out: {len(flows)} page(s) in parallel", flush=True)

    results = await asyncio.gather(
        *(_build_one_intent_page(flow, i) for i, flow in enumerate(flows))
    )
    ...
    lines = [f"{len(flows)} intent page(s) built in {total}s (parallel).", ""]
    for r in results:
        if r["ok"]:
...
that does not exist.
   - DERIVED & GUARDRAILS in every brief — three lines, "none" where the schema has none: the fields \
the flow auto-sets instead of asking (running-number pattern + the data it counts, start status key, \
today-dates); every stock/capacity/limit field a written quantity must be shown against and warned \
about (entity.field, verbatim from .entity_summary); per selection step ONE derived stat its list shows.
