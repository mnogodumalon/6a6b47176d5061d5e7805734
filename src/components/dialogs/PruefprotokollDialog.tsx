/**
 * PruefprotokollDialog — pre-generated create/edit dialog for Pruefprotokoll.
 *
 ...
 * PruefprotokollDialogDefaults — NOT the entity field type: lookup fields accept
 * the bare KEY string (or LookupValue), applookup fields the bare record id
 * (or record URL); the dialog normalizes. Type prefill STATE with the export:
 *  ❌ useState<Partial<Pruefprotokoll['fields']>>({ … })   // LookupValue fields reject string prefills (TS2322)
 *  ✓ useState<PruefprotokollDialogDefaults | undefined>(undefined)
 */
import { useState, useEffect, useMemo, useRef, useCallback } from 'react';
import type { Pruefprotokoll, Auftraege, LookupValue } from '@/types/app';
...
import { formEnhancements, computedDeps, computedApplookupRefs } from '@/config/form-enhancements/Pruefprotokoll';
...
/** Widened prefill type for PruefprotokollDialog.defaultValues — see file header. */
export type PruefprotokollDialogDefaults = Omit<Pruefprotokoll['fields'], 'pruefergebnis'> & {
    pruefergebnis?: LookupValue | string;
  };

interface PruefprotokollDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Pruefprotokoll['fields']) => Promise<void>;
  /** SHAPE-TOLERANT: lookup fields accept the bare key (string) or the
   *  LookupValue object; applookup fields the bare record id or the full
   *  record URL — the dialog normalizes both. */
  defaultValues?: PruefprotokollDialogDefaults;
...
const NORMALIZE_LOOKUPS: Record<string, readonly { key: string; label: string }[]> = {
  pruefergebnis: LOOKUP_OPTIONS['pruefprotokoll']?.['pruefergebnis'] ?? [],
};
...
export function PruefprotokollDialog({ open, onClose, onSubmit, defaultValues, recordId, auftraegeList, enablePhotoScan = true, enablePhotoLocation = true }: PruefprotokollDialogProps) {
  const [fields, setFields] = useState<Partial<Pruefprotokoll['fields']>>({});
  ...
  const REQUIRED_FIELDS = ['auftrag_pruef', 'monteur_name_vorname', 'monteur_name_nachname', 'pruefungsdatum', 'pruefergebnis'] as const;
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults(normalizedDefaults ?? {}, formEnhancements.defaults) as Partial<Pruefprotokoll['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'pruefprotokoll');
      await onSubmit(clean as Pruefprotokoll['fields']);
      onClose();
    } catch (err) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "auftrag_pruef": string | null, // Display name from Aufträge (see <available-records>)\n  "monteur_name_vorname": string | null, // Vorname des Monteurs\n  "monteur_name_nachname": string | null, // Nachname des Monteurs\n  "pruefungsdatum": string | null, // YYYY-MM-DDTHH:MM\n  "pruefergebnis": LookupValue | null, // Prüfergebnis (select one key: "nicht_bestanden" | "bestanden_mit_maengeln" | "bestanden") mapping: nicht_bestanden=Nicht bestanden, bestanden_mit_maengeln=Bestanden mit Mängeln, bestanden=Bestanden\n  "maengelbeschreibung": string | null, // Mängelbeschreibung\n  "massnahmen": string | null, // Eingeleitete Maßnahmen\n  "bemerkungen_pruef": string | null, // Bemerkungen\n}`;
      ...
      setFields(prev => {
        ...
        return merged as Partial<Pruefprotokoll['fields']>;
      });
  ...
  const DIALOG_INTENT = defaultValues ? 'Prüfprotokoll bearbeiten' : 'Prüfprotokoll hinzufügen';
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    'auftrag_pruef': (
      <div key="auftrag_pruef" className="space-y-1.5">
        ...
        <Combobox
          id="auftrag_pruef"
          placeholder="Welcher Auftrag wird geprüft?"
    ...
    'monteur_name_vorname': (
      <div key="monteur_name_vorname" className="space-y-1.5">
        <Label htmlFor="monteur_name_vorname">Vorname des Monteurs <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          id="monteur_name_vorname"
          placeholder="z. B. Hans"
          value={fields.monteur_name_vorname ?? ''}
          onChange={e => setFields(f => ({ ...f, monteur_name_vorname: e.target.value }))}
          required
        />
        {showErrors && !fields.monteur_name_vorname && (
          <p className="text-xs text-destructive mt-1">Pflichtfeld</p>
        )}
      </div>
    ),
    'monteur_name_nachname': (
      <div key="monteur_name_nachname" className="space-y-1.5">
        <Label htmlFor="monteur_name_nachname">Nachname des Monteurs <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          id="monteur_name_nachname"
          placeholder="z. B. Schulz"
          value={fields.monteur_name_nachname ?? ''}
          onChange={e => setFields(f => ({ ...f, monteur_name_nachname: e.target.value }))}
          required
        />
        {showErrors && !fields.monteur_name_nachname && (
          <p className="text-xs text-destructive mt-1">Pflichtfeld</p>
        )}
      </div>
    ),
    'pruefungsdatum': (
      <div key="pruefungsdatum" className="space-y-1.5">
        <Label htmlFor="pruefungsdatum">Datum der Prüfung <span className="text-destructive" aria-hidden="true">*</span></Label>
        <DatePicker
          id="pruefungsdatum"
          placeholder="Wann wurde geprüft?"
          mode="datetime"
          value={fields.pruefungsdatum ?? null}
          onChange={v => setFields(f => ({ ...f, pruefungsdatum: v ?? undefined }))}
          required
        />
        {showErrors && !fields.pruefungsdatum && (
          <p className="text-xs text-destructive mt-1">Pflichtfeld</p>
        )}
      </div>
    ),
    'pruefergebnis': (
      <div key="pruefergebnis" className="space-y-1.5">
        <Label htmlFor="pruefergebnis">Prüfergebnis <span className="text-destructive" aria-hidden="true">*</span></Label>
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.pruefergebnis) === 'nicht_bestanden'}
            onClick={() => setFields(f => ({ ...f, pruefergebnis: (lookupKey(f.pruefergebnis) === 'nicht_bestanden' ? undefined : 'nicht_bestanden') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.pruefergebnis) === 'nicht_bestanden'
          ...
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.pruefergebnis) === 'bestanden_mit_maengeln'}
            onClick={() => setFields(f => ({ ...f, pruefergebnis: (lookupKey(f.pruefergebnis) === 'bestanden_mit_maengeln' ? undefined : 'bestanden_mit_maengeln') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.pruefergebnis) === 'bestanden_mit_maengeln'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
          ...
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.pruefergebnis) === 'bestanden'}
            onClick={() => setFields(f => ({ ...f, pruefergebnis: (lookupKey(f.pruefergebnis) === 'bestanden' ? undefined : 'bestanden') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.pruefergebnis) === 'bestanden'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
        ...
        {showErrors && !fields.pruefergebnis && (
          <p className="text-xs text-destructive mt-1">Pflichtfeld</p>
        )}
      </div>
    ),
    'maengelbeschreibung': (
      <div key="maengelbeschreibung" className="space-y-1.5">
        ...
        <Textarea
          ...
          placeholder="Was war fehlerhaft? Beschreib Lage, Art und Umfang des Mangels."
    ...
    'massnahmen': (
      <div key="massnahmen" className="space-y-1.5">
        <Label htmlFor="massnahmen">Eingeleitete Maßnahmen</Label>
        <Textarea
          id="massnahmen"
          placeholder="Welche Korrektionen wurden eingeleitet? Zeitrahmen, Kosten, Zuständigkeit?"
    ...
    'fotos': (
      <div key="fotos" className="space-y-1.5">
        <Label htmlFor="fotos">Fotos / Anhänge</Label>
        {fields.fotos ? (
    ...
    'bemerkungen_pruef': (
      <div key="bemerkungen_pruef" className="space-y-1.5">
        ...
        <Textarea
          ...
          placeholder="Zusätzliche Anmerkungen zur Qualitätskontrolle, Nachfolgeprüfungen..."
  ...
  const FIELD_LABELS: Record<string, string> = {"auftrag_pruef": "Auftrag", "monteur_name_vorname": "Vorname des Monteurs", "monteur_name_nachname": "Nachname des Monteurs", "pruefungsdatum": "Datum der Prüfung", "pruefergebnis": "Prüfergebnis", "maengelbeschreibung": "Mängelbeschreibung", "massnahmen": "Eingeleitete Maßnahmen", "fotos": "Fotos / Anhänge", "bemerkungen_pruef": "Bemerkungen"};
  const CURRENCY_KEYS = new Set<string>([]);
  // Applookup-Referenz-Labels: pro applookup-Feld in dieser Form (ownKey)
  // eine Map { lookupKey: label } für ALLE Felder des Target-Schemas. Wird
  // beim Render-Walk gefiltert auf die in der computed-Formel tatsächlich
  // referenzierten lookupKeys (siehe applookupRefs unten).
  const APPLOOKUP_LABELS: Record<string, Record<string, string>> = {"auftrag_pruef": {"auftragsnummer": "Auftragsnummer", "auftragsdatum": "Auftragsdatum", "status": "Status", "prioritaet": "Priorität", "kunde": "Kunde", "auftragsbeschreibung": "Auftragsbeschreibung", "wunschtermin": "Wunschtermin", "liefertermin": "Liefertermin", "monteur": "Zuständiger Monteur", "auftrag_notizen": "Notizen"}};
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 py-4 space-y-4 min-w-0">
            ...
            {recordId && (
              <div className="pt-2 border-t border-border">
                <AttachmentsSection appId={APP_IDS.PRUEFPROTOKOLL} recordId={recordId} />
              </div>
            )}
          </div>
          {submitError && (
