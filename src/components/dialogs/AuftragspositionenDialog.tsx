 ...
 * materialList (full hook array — resolves the Material applookup),
...
import type { Auftragspositionen, Auftraege, Material, LookupValue } from '@/types/app';
...
export type AuftragspositionenDialogDefaults = Omit<Auftragspositionen['fields'], 'einheit_position'> & {
    einheit_position?: LookupValue | string;
  };
...
const NORMALIZE_LOOKUPS: Record<string, readonly { key: string; label: string }[]> = {
  einheit_position: LOOKUP_OPTIONS['auftragspositionen']?.['einheit_position'] ?? [],
};
const NORMALIZE_APPLOOKUPS: Record<string, string> = {
  auftrag: APP_IDS.AUFTRAEGE,
  material: APP_IDS.MATERIAL,
};
...
export function AuftragspositionenDialog({ open, onClose, onSubmit, defaultValues, recordId, auftraegeList, materialList, enablePhotoScan = true, enablePhotoLocation = true }: AuftragspositionenDialogProps) {
  ...
  // Inline-Create state for "Material" target. The dropdown's
  // "+ Neuer …" option opens a sub-dialog; on submit we POST, add the new
  // record to the local `extraMaterial` list, and select it in
  // the originating Combobox via the captured `createMaterialField`.
  const [createMaterialOpen, setCreateMaterialOpen] = useState(false);
  const [createMaterialInitial, setCreateMaterialInitial] = useState('');
  const [createMaterialField, setCreateMaterialField] = useState<string>('');
  const [extraMaterial, setExtraMaterial] = useState< Material[]>([]);
  const materialListAll = useMemo(
    () => [...materialList, ...extraMaterial],
    [materialList, extraMaterial],
  );
  function openCreateMaterial(fieldKey: string, q: string) {
    setCreateMaterialField(fieldKey);
    setCreateMaterialInitial(q);
    setCreateMaterialOpen(true);
  }
  const [showErrors, setShowErrors] = useState(false);
  const REQUIRED_FIELDS = ['auftrag', 'material', 'menge'] as const;
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      contextParts.push(`<available-records field="material" entity="Material">\n${JSON.stringify(materialList.map(r => ({ record_id: r.record_id, ...r.fields })), null, 2)}\n</available-records>`);
      if (usePersonalInfo) {
        try {
      ...
      const schema = `{\n  "auftrag": string | null, // Display name from Aufträge (see <available-records>)\n  "material": string | null, // Display name from Material (see <available-records>)\n  "positionsbeschreibung": string | null, // Positionsbeschreibung\n  "menge": number | null, // Menge\n  "einheit_position": LookupValue | null, // Einheit (select one key: "stueck" | "meter" | "liter" | "kilogramm" | "paket" | "rolle" | "satz") mapping: stueck=Stück, meter=Meter, liter=Liter, kilogramm=Kilogramm, paket=Paket, rolle=Rolle, satz=Satz\n  "bemerkung": string | null, // Bemerkung\n}`;
      ...
      setFields(prev => {
        ...
        const applookupKeys = new Set<string>(["auftrag", "material"]);
        ...
        const materialName = raw['material'] as string | null;
        if (materialName) {
          const materialMatch = materialList.find(r => matchName(materialName!, [String(r.fields.bezeichnung ?? '')]));
          if (materialMatch) merged['material'] = createRecordUrl(APP_IDS.MATERIAL, materialMatch.record_id);
        }
  ...
  const DIALOG_INTENT = defaultValues ? 'Auftragspositionen bearbeiten' : 'Auftragspositionen hinzufügen';
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    'auftrag': (
      <div key="auftrag" className="space-y-1.5">
        ...
        <Combobox
          id="auftrag"
          placeholder="Zu welchem Auftrag gehört diese Position?"
    ...
    'material': (
      <div key="material" className="space-y-1.5">
        <Label htmlFor="material">Material <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Combobox
          id="material"
          placeholder="Welches Material wird benötigt?"
          items={materialListAll.map(r => ({
            id: r.record_id,
            label: String(r.fields.bezeichnung ?? r.record_id),
          }))}
          value={extractRecordId(fields.material)}
          onChange={id => setFields(f => ({ ...f, material: id ? createRecordUrl(APP_IDS.MATERIAL, id) : undefined }))}
          searchPlaceholder="Suchen…"
          emptyText="Kein Treffer"
          onCreateNew={(q) => openCreateMaterial("material", q)}
          createLabel="Neu in Material"
        />
        {showErrors && !fields.material && (
          <p className="text-xs text-destructive mt-1">Pflichtfeld</p>
        )}
      </div>
    ),
    'positionsbeschreibung': (
      <div key="positionsbeschreibung" className="space-y-1.5">
        <Label htmlFor="positionsbeschreibung">Positionsbeschreibung</Label>
        <Input
          id="positionsbeschreibung"
          placeholder="z. B. Stahlrahmen-Montage"
          value={fields.positionsbeschreibung ?? ''}
          onChange={e => setFields(f => ({ ...f, positionsbeschreibung: e.target.value }))}
        />
      </div>
    ),
    'menge': (
      <div key="menge" className="space-y-1.5">
        <Label htmlFor="menge">Menge <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          id="menge"
          type="number"
          step="any"
    ...
    'einheit_position': (
      <div key="einheit_position" className="space-y-1.5">
        <Label htmlFor="einheit_position">Einheit</Label>
        <Select
          value={lookupKey(fields.einheit_position) ?? ''}
          onValueChange={v => setFields(f => ({ ...f, einheit_position: v === 'none' ? undefined : v as any }))}
        >
          <SelectTrigger id="einheit_position" className="max-sm:h-11"><SelectValue placeholder="Mengeneinheit wählen" /></SelectTrigger>
          <SelectContent>
    ...
    'bemerkung': (
      <div key="bemerkung" className="space-y-1.5">
        ...
        <Textarea
          id="bemerkung"
          placeholder="Besonderheiten, Montagehinweise, Qualität, Lagerort des Materials..."
  ...
  const FIELD_LABELS: Record<string, string> = {"auftrag": "Auftrag", "material": "Material", "positionsbeschreibung": "Positionsbeschreibung", "menge": "Menge", "einheit_position": "Einheit", "bemerkung": "Bemerkung"};
  const CURRENCY_KEYS = new Set<string>([]);
  // Applookup-Referenz-Labels: pro applookup-Feld in dieser Form (ownKey)
  // eine Map { lookupKey: label } für ALLE Felder des Target-Schemas. Wird
  // beim Render-Walk gefiltert auf die in der computed-Formel tatsächlich
  // referenzierten lookupKeys (siehe applookupRefs unten).
  const APPLOOKUP_LABELS: Record<string, Record<string, string>> = {"auftrag": {"auftragsnummer": "Auftragsnummer", "auftragsdatum": "Auftragsdatum", "status": "Status", "prioritaet": "Priorität", "kunde": "Kunde", "auftragsbeschreibung": "Auftragsbeschreibung", "wunschtermin": "Wunschtermin", "liefertermin": "Liefertermin", "monteur": "Zuständiger Monteur", "auftrag_notizen": "Notizen"}, "material": {"bezeichnung": "Materialbezeichnung", "artikelnummer": "Artikelnummer", "einheit": "Einheit", "lagerbestand": "Lagerbestand (Menge)", "mindestbestand": "Mindestbestand", "verfuegbarkeit": "Verfügbarkeit", "material_notizen": "Notizen"}};
  ...
  return (
    ...
    {createMaterialOpen && (
      <MaterialDialog
        ...
        onSubmit={async (newFields) => {
          const result = await LivingAppsService.createMaterialEntry(newFields as any) as { id?: string };
          if (result?.id) {
            const newRec = { record_id: result.id, fields: newFields } as unknown as Material;
            setExtraMaterial(prev => [...prev, newRec]);
            const url = createRecordUrl(APP_IDS.MATERIAL, result.id);
            setFields(prev => ({ ...prev, [createMaterialField]: url } as any));
          }
          setCreateMaterialOpen(false);
        }}
        defaultValues={createMaterialInitial
          ? ({ bezeichnung: createMaterialInitial } as any)
          : undefined}
      />
    )}
    </>
