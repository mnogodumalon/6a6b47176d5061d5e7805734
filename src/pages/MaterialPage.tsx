...
export default function MaterialPage() {
  ...
  async function loadData() {
    setLoading(true);
    try {
      setRecords(await LivingAppsService.getMaterial());
    } finally {
      setLoading(false);
    }
  }

  async function handleCreate(fields: Material['fields']) {
    await LivingAppsService.createMaterialEntry(fields);
    await loadData();
    setDialogOpen(false);
  }

  async function handleUpdate(fields: Material['fields']) {
    if (!editingRecord) return;
    await LivingAppsService.updateMaterialEntry(editingRecord.record_id, fields);
    await loadData();
  ...
  async function handleDelete() {
    ...
    await LivingAppsService.deleteMaterialEntry(deleteTarget.record_id);
  ...
  return (
    <PageShell
      title="Material"
      subtitle={`${records.length} Material im System`}
      action={
        ...
        <Table className="[&_tbody_td]:px-6 [&_tbody_td]:py-2 [&_tbody_td]:text-base [&_tbody_td]:font-medium [&_tbody_tr:first-child_td]:pt-6 [&_tbody_tr:last-child_td]:pb-10">
          <TableHeader className="bg-secondary">
            <TableRow className="border-b border-input">
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('artikelnummer')}>
                <span className="inline-flex items-center gap-1">
                  Artikelnummer
                  {sortKey === 'artikelnummer' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('material_notizen')}>
                <span className="inline-flex items-center gap-1">
                  Notizen
                  {sortKey === 'material_notizen' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
          ...
          <TableBody>
            {sortRecords(filtered).map(record => (
              <TableRow key={record.record_id} className="hover:bg-muted/50 transition-colors cursor-pointer" onClick={(e) => { if ((e.target as HTMLElement).closest('button, [role="checkbox"]')) return; navigate(`/material/${record.record_id}`); }}>
                <TableCell className="font-medium">{record.fields.bezeichnung ?? '—'}</TableCell>
                <TableCell>{record.fields.artikelnummer ?? '—'}</TableCell>
                ...
                <TableCell className="max-w-xs"><span className="truncate block">{record.fields.material_notizen ?? '—'}</span></TableCell>
                <TableCell>
            ...
            {filtered.length === 0 && (
              <TableRow>
                <TableCell colSpan={8} className="text-center py-16 text-muted-foreground">
                  {search ? 'Keine Ergebnisse gefunden.' : 'Noch keine Material. Jetzt hinzufügen!'}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
