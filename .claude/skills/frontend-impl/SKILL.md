...
- `DashboardSkeleton`/`DashboardError` (imported from `@/components/DashboardStates`, self-repair flow inside) — keep the import and the two early-returns; NEVER re-implement them locally (check-dashboard gate 20)
...
const labelOf = (key: string) => STAGES.find(o => o.key === key)?.label ?? key;
...
const advance = useCallback((r: <Entity>) => {
  ...
  set<Entity>(prev => prev.map(x => x.record_id === r.record_id
    ? { ...x, fields: { ...x.fields, <statusfeld>: { key: to, label: labelOf(to) } } } : x));
  update<Entity>Entry(r.record_id, { <statusfeld>: to }).catch(() => fetchAll());
}, [set<Entity>, fetchAll]);
```
`labelOf` is why this is a helper and not three inline handlers: the optimistic write must carry the LABEL from the schema. Hard-coding the stage keys (`['offen', 'in_bearbeitung', …]`) instead of reading `LOOKUP_OPTIONS` re-introduces the invented-stage-name bug — the order is already in the schema.
...
```
Prefill STATE: `useState<EntityDialogDefaults | undefined>()` (exported by each dialog) — NEVER `Partial<Entity['fields']>` (its LookupValue fields make string prefills a TS2322).
