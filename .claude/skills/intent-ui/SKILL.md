...
- **No `toISOString()`**: format dates locally — see "Dates" below. Checked by `check-intents.mjs`.
...
### Optional selects: the sentinel, both halves
...
```
Both halves matter: the first keeps the dropdown from crashing, the second keeps `'none'` out of the
record — it is not a lookup key, so the API would reject it or store nonsense.
...
`node scripts/check-intents.mjs` flags a `toISOString()` in a flow page.
