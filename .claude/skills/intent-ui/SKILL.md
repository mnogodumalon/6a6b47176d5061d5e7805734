...
const [state, setState] = useState({ eventId: '', gaeste: [] as string[], notiz: '' });

<IntentWizardShell
  title="Event vorbereiten"
  subtitle="Schritt-für-Schritt zum perfekten Event"
  steps={[{label: 'Event'}, {label: 'Gäste'}, {label: 'Prüfen'}, {label: 'Fertig'}]}
  currentStep={step}
  ...
  onRetry={fetchAll}
  intro={{
    description: 'Legt das Event an, lädt Gäste ein und reserviert die Räume.',
    requirements: ['Eventdatum', 'Gästeliste'],
  }}
  answers={step > 1 && selectedEvent ? [{ label: 'Event', value: eventName(selectedEvent) }] : undefined}
  draftKey="intent:event-vorbereiten"
  draft={state}
  onDraftRestore={(d) => setState(d as typeof state)}
>
  {step === 1 && <EventSelect ... />}
  {step === 2 && <GuestInvite ... />}
  {step === 3 && <SummaryStep ... />}   {/* check-answers BEFORE the write — see below */}
  {step === 4 && <SuccessStep ... />}   {/* result + follow-up actions — see below */}
</IntentWizardShell>
```
Handles: start card, step indicator circles (visited circles are buttons — free jumps backwards),
"noch X Schritte" remaining count, URL deep-linking (?step=N), draft persistence with a resume hint,
the answers context bar, loading/error states, **and a back link in its header** (to `#/` unless you say
otherwise). Each step must provide its own action/navigation buttons (e.g., "Weiter zu Schritt 3") — but
do NOT add a second "Zurück zum Dashboard" link, the shell already renders one.
...
- `intro?: { description?, requirements?, startLabel? }` — the start card: what the flow does and what
  the user needs at hand. ALWAYS pass it. The shell lists the step labels itself; deep links and
  restored drafts skip the card automatically.
- `answers?: { label, value }[]` — the context bar: choices made so far, visible on every later step
  ("Kunde: Müller GmbH"). Pass the answers a later step builds on; recognition beats memory.
- `draftKey` / `draft` / `onDraftRestore` — draft persistence, see the block below. ALWAYS wire all three.

**Draft persistence: wire `draftKey` + `draft` + `onDraftRestore`, and clear on success.** Keep ALL
wizard answers in ONE state object, pass it as `draft`; the shell saves it (with the step) on every
change and restores both on the next visit — an interruption (phone call, dead spot, reload) costs
nothing, and a shared `?step=N` link lands on populated steps instead of blank ones. After the final
successful write call `clearIntentDraft(draftKey)`, or the next visit resumes a finished flow.

```tsx
// ❌ scattered state — nothing survives a reload, ?step=3 renders a blank body
const [eventId, setEventId] = useState(''); const [gaeste, setGaeste] = useState([]);

// ✅ one draft object, saved+restored by the shell; cleared after the write
const [state, setState] = useState({ eventId: '', gaeste: [] as string[] });
<IntentWizardShell draftKey="intent:event-vorbereiten" draft={state}
  onDraftRestore={(d) => setState(d as typeof state)} … >
// …in the final submit, after success: clearIntentDraft('intent:event-vorbereiten')
```

A step whose prerequisite is missing anyway (deep link, cleared storage) still needs the fallback: show
one line naming what is missing and a button back to the step that sets it — never a blank body.
...
It colours a table of known German status keys PLUS **every lookup key of THIS app** — the generator
appends the app's own keys with heuristic colours, so no real status falls through to grey. Never
rebuild or extend the table. When you need the same colour on your own element (a tile, a row, a chip),
take it from the same source instead of writing a second table:
...
### SummaryStep — check-answers BEFORE the write (gate-enforced)

Every flow shows the collected answers on ONE step before anything is written.
The user reviews, jumps back to fix, confirms — the confirm button does the
write. `check-intents` fails a flow page without `<SummaryStep>`.

```tsx
import { SummaryStep } from '@/components/blocks/SummaryStep';
import { useIntentSubmit } from '@/hooks/useIntentSubmit';

const { submit, submitting, error, done } = useIntentSubmit(async () => {
  const auftrag = await LivingAppsService.createAuftraegeEntry({ … });
  clearIntentDraft('intent:auftrag-anlegen');
  return auftrag;
});
...
{step === 3 && !done && (
  <SummaryStep
    items={[
      { label: 'Kunde', value: kundeName(sel), step: 1 },
      { label: 'Positionen', value: `${state.positionen.length} Positionen · ${formatCurrency(summe)}`, step: 2 },
      { label: 'Nummer', value: nummer },          // derived — no step, nothing to edit
    ]}
    onEdit={setStep}
    whatHappensNext="Der Auftrag geht an die Disposition; der Kunde erhält eine Bestätigung."
    confirmLabel="Auftrag anlegen"
    onConfirm={async () => { if (await submit()) setStep(4); }}
...
Props:
- `items` — `{ label, value, step? }[]`: one row per answer. `step` renders the "Ändern" jump back to
  the step that owns the value; omit it on derived rows (nothing to edit there).
- `onEdit` — pass `setStep`; the row's link calls it
- `whatHappensNext?` — ONE sentence of process consequence. You know the process from the brief — write it.
- `confirmLabel` / `onConfirm` / `submitting?` — the write, driven by `useIntentSubmit` (below)
- `error?` / `onRetry?` — a failed write keeps the entered data and renders an inline retry row
  (`onRetry` defaults to `onConfirm`)
- `title?` — defaults to the localized "Alles richtig?"

### SuccessStep — the flow's last step (gate-enforced)

After the write the flow does NOT navigate back to a list. It names the result
(with its number), lists what it caused, and offers the next actions — the
FIRST action is the primary button, and it should be the next logical step of
the work. `check-intents` fails a flow page without `<SuccessStep>`.
...
{step === 4 && (
  <SuccessStep
    title={`Auftrag ${nummer} angelegt`}
    details={[`${state.positionen.length} Positionen reserviert`, 'Status „Offen“ gesetzt']}
    actions={[
      { label: 'Weiter: Material bestellen', href: '#/intents/material-bestellen' },
      { label: 'Weiteren Auftrag anlegen', onClick: resetWizard },
...
Props: `title` (name the result — include the number), `details?` (one fact per line), `actions?`
(2–3 of `{ label, href?, onClick?, icon? }`; allowed hrefs: `#/` and `#/intents/<slug>` only).

### useIntentSubmit — submit state for the final write

```tsx
import { useIntentSubmit } from '@/hooks/useIntentSubmit';
```

Wraps the flow's write callback and owns the three rules every flow needs:
a retry guard (a second click never fires the write again — the create cannot
duplicate), error-as-state (feed `error` into SummaryStep; the inputs stay),
and the success transition (`done` flips / `submit()` resolves truthy → show
the SuccessStep). Returns `{ submit, submitting, error, done, result, reset }`;
`reset` backs a "weiteren Auftrag anlegen" action. Chained creates stay inside
the ONE callback — store each `record_id` as you go (idempotency, see below).
...
## UX Floor (MANDATORY — what separates a guided flow from a chopped-up form)

Six rules. The brief's "Derived & guardrails" section names the concrete
fields — wire every one it names.

### 0. The skeleton is fixed (gate-enforced)

Every flow: start card (`intro`) → steps, one decision each → `<SummaryStep>`
BEFORE the write → `<SuccessStep>` after it, plus `draftKey`/`draft`/`onDraftRestore`.
Your creativity lives INSIDE the steps — what to ask, what to derive, what to
warn about — not in the frame around them.

### 1. Never ask what the system can derive

A wizard that makes the user type a running number, pick the status of a
brand-new record, or enter today's date is just the CRUD dialog cut into steps.
Auto-set every derivable field, show it as read-only context, ask only real
questions.

❌ DON'T (create step):
```tsx
<Input value={nummer} onChange={…} placeholder="z. B. BU-2026-001" />
<Select value={status}>…every status option…</Select>
```

✅ DO:
```tsx
// Running number derived from the data; start status written without asking.
const nummer = `BU-${format(new Date(), 'yyyyMMdd')}-${String(buchungen.length + 1).padStart(3, '0')}`;
<p className="text-sm text-muted-foreground">Nummer <b>{nummer}</b> · Status „Angefragt“</p>
// …later: createBuchungEntry({ nummer, status: 'angefragt', … })
```

The start status is the FIRST key of the status lookup unless the brief names
one. Count-based numbers can collide when two users create at once — acceptable,
the number is context, not a database key; never invent a fancier scheme.

### 2. Numeric guardrails: show and warn, don't just collect

When a step writes a quantity/amount and a referenced record carries a stock,
capacity or limit number, the input must show that value and WARN — not
silently block — when exceeded.

❌ DON'T: a bare `<Input type="number" />` with the stock nowhere on screen.

✅ DO:
```tsx
<Input type="number" value={menge} onChange={…} />
<p className="text-xs text-muted-foreground">Auf Lager: {sel.fields.bestand ?? 0}</p>
{Number(menge) > (sel.fields.bestand ?? 0) && (
  <p className="text-xs text-destructive">Übersteigt den Bestand — trotzdem möglich.</p>
)}
```

### 3. Selection lists carry context, not just names

Every `EntitySelectStep` shows at least one DERIVED value per item via `stats`
(count of linked records, a stock level, open items) so the user can decide
without leaving the step.

❌ DON'T: `items={gaeste.map(g => ({ id: g.record_id, title: gastName(g) }))}` — a bare name list.
...
items={gaeste.map(g => ({
  id: g.record_id,
  title: gastName(g),
  stats: [{ label: 'Offene Buchungen', value: buchungen.filter(b =>
    extractRecordId(b.fields.gast) === g.record_id &&
    lookupKey(b.fields.status) !== 'abgereist').length }],
}))}
```

### 4. Recall: later steps speak in earlier answers

Once the user picked something, every later step text uses it BY NAME — the
flow proves it listened. Interpolate previous answers into step headings and
questions, and pass them to the shell's `answers` bar.

❌ DON'T: `<h2>Termin wählen</h2>` on step 3 of a flow that picked a customer on step 1.

✅ DO:
```tsx
<h2>Wann soll das Team bei {kundeName(sel)} sein?</h2>
```

### 5. Reversible actions get undo, not a confirm dialog

Removing an item from a step's list, unpicking a record — do it immediately
and offer undo via the pre-built toast. A confirm dialog is ONLY for
irreversible writes, and then it names the consequence.

❌ DON'T: `<ConfirmDialog title="Sind Sie sicher?" …>` around removing a position from local state.
...
undoToast('Position entfernt', () => setState(s => ({ ...s, positionen: [...s.positionen, removed] })));
```
...
- **The final write runs through `useIntentSubmit`** — it owns the double-click guard, error-as-state and the success transition; hand-rolled `submitting` flags re-introduce the bugs it exists to prevent.
- **A step that `create`s more than one record must not duplicate them on a retry** — see "survive its own retry" below. The stored ids gate the creates INSIDE the submit callback; without them a retry after a partial failure writes the first record again.
...
`useIntentSubmit` already blocks the double-CLICK (its guard never fires the callback twice in
parallel). What it cannot know is which records of a PARTIALLY failed chain already exist — that is
what the stored ids are for, INSIDE the callback. The id you already store for the summary IS the
idempotency key. Read it before writing, don't only display it:
